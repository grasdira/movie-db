import type { TMDBMovieDetailFull } from '@/lib/api/types';
import type {
  MovieDetail,
  Cast,
  Crew,
...
/**
 * MovieDetailAdapter 負責將 TMDB API 的電影詳細資訊轉換成應用程式內部使用的格式
 */
export class MovieDetailAdapter {
  ...
  static toMovieDetail(response: TMDBMovieDetailFull): MovieDetail {
    return {
      ...
      posterUrl: this.getImageUrl(response.poster_path, 'w500'),
      backdropUrl: this.getImageUrl(response.backdrop_path, 'w1280'),
